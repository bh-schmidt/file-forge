...
export class ForgeRunner {
    ...
    async init() {
        ...
        await this.forge.program._configureCommands(this.forge.program._commander)
        this.forge.program._commander.parse()

        const opts = this.forge.program._commander.opts()

        const commandsResult = await this.forge.program._validateCommands(this.forge.program._commander)
        if (commandsResult !== undefined && commandsResult !== true) {
            this.forge.program._commander.error(`error: ${commandsResult}`)
        }

        for (const option of this.forge.program._commander.options) {
            ...
            this.forge.program._commander.error(`error: option '${option.flags}' is invalid. ${result}`)
        }
    ...
    async rollback() {
        ...
        if (this.forge.memFs.isDisposed()) {
            console.log(chalk.yellow.bold(`Changes were already committed, couldn't rollback.`))
        } else {
